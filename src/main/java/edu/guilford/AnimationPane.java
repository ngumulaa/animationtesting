...
public class AnimationPane extends Pane {
     // we choose Pane so that we cna place and move objects anywhere we want 
     ...
     // Let's add a Circle to the Pane at a random location 
        Circle circle = new Circle(50,Color.INDIGO); 
        circle.setCenterX(Math.random() * width);
        circle.setCenterY(Math.random() * height);
        this.getChildren().add(circle); 

     // JavaFX animations are based on transitions: a starting point, an ending point, and a duration
     // Example: fade the circle from 1.0 (opaque) to 0.0 (transparent) over 2 seconds
     // Build a FadeTransition object (from the animation package) with the circle as the target 
     ...
     // Now we can build a PathTransition object with this path and the circle as the target 
      ...
      // That's done with the a KeyFrame and one or more KeyValues 
     ...
     // Let's add a Circle to the Pane at a random location
        circEvent = new Circle(50,Color.GREEN);
     ...
     // inner class to animate the circle: it's and EventHandler interface specifically for ActionEvents
        private class CircAnimate implements EventHandler<ActionEvent> { 
            ...
            public void handle(ActionEvent arg0) { 
                ...
                // checck if the circle is the near the edge of the pane, and it it is reverse the velocity 
